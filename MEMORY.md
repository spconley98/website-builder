...
**Last updated:** 2026-06-07T14:40:00-05:00 · **Last agent:** Gemini (Antigravity) — Matt original Places API key verification
...
- Google Places API (Matt) — key returns `LegacyApiNotActivatedMapError`. Matt needs to enable the legacy "Places API" (not just the new one) in his Google Cloud Console for the project linked to the original key.
